...
> **Status: IMPLEMENTED** (owner-reviewed and merged to `main` 2026-06-13, PR #5, merge commit `1fa8b76`). design-quality-gate (Tier T2) **CLEARED** — 14 original + 4 re-review findings (R-001..R-004) + 4 post-merge format-consumer findings (PMR-001..004) resolved across **three** independent adversarial re-gates; the format-change blast radius is enumerated in **C.9** and covered by **Phase 7**.
>
> **Implementation complete on branch `proposal/session-persistence-v2`** (2026-06-13). All phases (Tasks 0–23) built test-first; the full Python suite is green (1365 passed, 7 skipped) and `ruff check src tests scripts` is clean. One independent code review was run against C.1–C.9 and its findings resolved (corrupt-v1 tolerance, the listing backfill's read-modify-write under lock, CONFLICT supersede, ohmo index fsync policy, head-less backfill). A subsequent **owner review** found three further issues — the v1-revert switch left stale v2 files shadowing the new v1 for by-id loads; the C.6 v2-pointer → legacy-v1 fallback was unimplemented; and `append_jsonl_line` did not fsync the parent dir when *creating* a new transcript — all three now fixed (openharness + ohmo) and covered by tests. The Phase 7 consumer fixes (PMR-001..004) were interleaved with the phases that introduce v2-as-default rather than deferred to the end, so the suite stays green at every commit. **Merged to `main` on 2026-06-13** (PR #5, merge commit `1fa8b76`) after owner review — status **IMPLEMENTED**; the full suite (1365 passed, 7 skipped) was re-verified green on the merge result. Branch archival (rename `proposal/session-persistence-v2` → `archive/proposal/session-persistence-v2` per AGENTS §4) remains as the final lifecycle step.
...
- **Idempotent:** re-running yields the same index. Entries are keyed by `session_id` and re-derived from each session's head (v2), the v2 transcript via the loader for **head-less v2** (no head survived the crash — C.6, so the entry is loader-derived with degraded `model`/`summary`), or the full file (v1); existing entries are never duplicated.
...
1. - [ ] Failing tests. **(a) Backend unit** (`tests/test_services/test_session_storage.py`):
   ...
   **(b) Command-level regression** (`tests/test_commands/test_registry.py` — the PMR-001 binding proof; mirrors that file's `_make_context`/`registry.lookup` pattern):
   ...
   async def test_session_tag_command_exports_full_snapshot_under_v2(tmp_path: Path, monkeypatch):
       ...
       registry = create_default_command_registry()        # same construction the other tests in this file use
...
> **⚠ SCOPE — this is a DESIGN gate, not an implementation review.** **No v2 implementation code exists yet, by design** — `session_format.py` is not created, `save_session_snapshot` is still v1-only, and the tests below are **specified TDD steps that run *at* implementation, not now.** "Resolved" / "CLEARED" / "implementation-ready" mean the *design and its test spec* resolve the finding — **not** that code is written. So "function X is absent" / "test Y is not in the tree" / "the code is still v1" are the **expected, correct** state of this DRAFT plan and are **NOT** gate findings. (A multi-agent audit on 2026-06-13 had several agents mis-flag exactly this as "P1"; it is not.) A real gate finding is a flaw in the *design or its test specification* — e.g. a task whose specified test could pass while its contract is violated.
...
| C.2 v2 save acquires `.sessions.lock` exactly once and runs the lock-free index core once **while the lock is held**, never the locking wrapper (no nested flock, no unlocked write outside the lock) | behavioral | `test_v2_save_uses_unlocked_index_core_not_the_locking_wrapper` |
| C.7 backfill writes the index **exactly once inside exactly one `.sessions.lock` acquisition** (atomic all-or-nothing, not once-per-entry, not unlocked) | behavioral | `test_backfill_writes_index_exactly_once` |
...
- [x] Tests / verification cover every P1/P2 class found. — original 14 (Q.3.1), the three R-fix regression tests (`fingerprint_messages` in-place change, same-count in-place rewrite, ohmo lost-head recovery), **and the four PMR regression tests** (command-level `/session tag` export, index-missing v1+v2/head-less listing with persisted backfill, conversation-index v2 rebuild, auto-dream v2 discovery) — all registered in Q.3.1.
