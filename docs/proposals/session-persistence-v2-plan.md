...
This remains a **design** gate: the tests above are *specified* as TDD steps to run at implementation, not executed as part of the gate. **The plan stays DRAFT** — clearing the gate is a prerequisite for, not the act of, promotion. Promotion to APPROVED and any merge of this branch to `main` are the owner's decision (main still holds the pre-revision plan; nothing is merged).
