...
@pytest.mark.asyncio
async def test_headless_search_never_hangs_even_if_index_blocks(tmp_path: Path, monkeypatch):
    """If the index layer ever blocks, the protocol answers with a
    diagnosable error instead of hanging — the structural fix for the
    reported (locally unreproducible) first-run hang."""
    import asyncio
    import threading

    monkeypatch.setenv("OPENHARNESS_CONFIG_DIR", str(tmp_path / "config"))
    monkeypatch.setenv("OPENHARNESS_DATA_DIR", str(tmp_path / "data"))
    import openharness.services.conversation_index as ci
    from openharness.api.client import ApiMessageCompleteEvent
    from openharness.api.usage import UsageSnapshot
    from openharness.engine.messages import ConversationMessage, TextBlock
    from openharness.ui.app import run_headless_control
    ...
    class _BlockingIndex:
        def search(self, *a, **k):
            release.wait(timeout=30)  # simulates an indefinite filesystem block
            return {"hits": []}

    monkeypatch.setattr(ci, "get_conversation_index", lambda: _BlockingIndex())
    # Patch the symbol the handler imports at call time too.
    monkeypatch.setattr(
        "openharness.services.conversation_index.INDEX_OPERATION_TIMEOUT_SECONDS", 0.5
    )
